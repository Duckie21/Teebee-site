...
// Replace the inline updater with two named updaters: static and live
// (moved from rain.php). This avoids name collisions and lets the page
// decide which updater to run.

// Static leaderboard updater (reads a pre-generated JSON file)
async function updateLeaderboardStatic() {
  ...
  try {
    // Fetch the static JSON generated by the scheduled script (cache-busted)
    const response = await fetch('/Teebee-site/assets/data/rain.json?ts=' + Date.now());
...
// Live updater used on the dedicated `rain.php` page. Polls the server API endpoint
// every second to provide near-real-time updates.
async function updateLeaderboardLive() {
  if (!leaderboardContainer) return;

  try {
    const response = await fetch('/Teebee-site/rain.php?api=1');
    const data = await response.json();
    ...
    leaderboardContainer.innerHTML = html;
    console.log('Leaderboard updated successfully (live)');
  } catch (err) {
    console.error('Failed to update leaderboard (live):', err);
  }
}

// Decide which updater to run: live on the rain.php page, static elsewhere
if (leaderboardContainer) {
  const isRainPage = window.location.pathname.endsWith('/rain.php') || window.location.pathname.includes('/rain.php');

  if (isRainPage) {
    // Live updates (short interval) for the dedicated leaderboard page
    updateLeaderboardLive();
    setInterval(updateLeaderboardLive, 1000);
  } else {
    // Static (cached JSON) updates for other pages
    updateLeaderboardStatic();
    // Poll every 10s to avoid excessive requests. The static JSON is regenerated by a scheduled task.
